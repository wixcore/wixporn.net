...
if ( isset( $_POST['save'] ) ) {
    if ( isset( $_POST['nick'] ) && $_POST['nick'] != $ank['nick'] ) {
        if ( db::count("SELECT COUNT(*) FROM `user` WHERE `nick` = '" . my_esc( $_POST['nick'] ) . "'") == 1 )
        ...
        elseif ( user_access( 'user_change_nick' ) ) {
            ...
            if ( !isset( $err ) ) {
                ...
                $ank['nick'] = $nick;
                db::query("UPDATE `user` SET `nick` = '$nick' WHERE `id` = '$ank[id]' LIMIT 1");
            }
        } else
    ...
    if ( isset( $_POST['set_show_icon'] ) && ( $_POST['set_show_icon'] == 1 || $_POST['set_show_icon'] == 0 ) ) {
        ...
        db::query("UPDATE `user` SET `set_show_icon` = '$ank[set_show_icon]' WHERE `id` = '$ank[id]' LIMIT 1");
    } else
    ...
    if ( isset( $_POST['set_translit'] ) && ( $_POST['set_translit'] == 1 || $_POST['set_translit'] == 0 ) ) {
        ...
        db::query("UPDATE `user` SET `set_translit` = '$ank[set_translit]' WHERE `id` = '$ank[id]' LIMIT 1");
    } else
    ...
    if ( isset( $_POST['set_time_chat'] ) && ( is_numeric( $_POST['set_time_chat'] ) && $_POST['set_time_chat'] >= 0 && $_POST['set_time_chat'] <= 900 ) ) {
        ...
        db::query("UPDATE `user` SET `set_time_chat` = '$ank[set_time_chat]' WHERE `id` = '$ank[id]' LIMIT 1");
    } else
    ...
    if ( isset( $_POST['ank_skype'] ) && preg_match( '#^([A-z0-9 \-]*)$#ui', $_POST['ank_skype'] ) ) {
        ...
        if ( $ank['ank_skype'] == null )
            $ank['ank_skype'] = 'null';
        db::query("UPDATE `user` SET `ank_skype` = '" . my_esc( $ank['ank_skype'] ) . "' WHERE `id` = '$ank[id]' LIMIT 1");
    } else
    ...
    if ( isset( $_POST['ank_o_sebe'] ) && preg_match( '#^([A-zА-я \-]*)$#ui', $_POST['ank_o_sebe'] ) ) {
        ...
        db::query("UPDATE `user` SET `ank_o_sebe` = '$ank[ank_o_sebe]' WHERE `id` = '$ank[id]' LIMIT 1");
    } else
    ...
    if ( user_access( 'user_change_group' ) && isset( $_POST['group_access'] ) ) {
        if ( db::count("SELECT COUNT(*) FROM `user_group` WHERE `id` = '" . intval( $_POST['group_access'] ) . "' AND `level` < '$user[level]'") == 1 ) {
            if ( $ank['group_access'] != intval( $_POST['group_access'] ) ) {
                admin_log( 'Пользователи', 'Изменение статуса', "Пользователь '$ank[nick]': Статус '$ank[group_name]' изменен на '" . db::count("SELECT `name` FROM `user_group` WHERE `id` = '" . intval( $_POST['group_access'] ) . "'") . "'" );
    ...
    if ( ( $user['level'] >= 3 || $ank['id'] == $user['id'] ) && isset( $_POST['balls'] ) && is_numeric( $_POST['balls'] ) ) {
        ...
        db::query("UPDATE `user` SET `balls` = '$ank[balls]' WHERE `id` = '$ank[id]' LIMIT 1");
    }
